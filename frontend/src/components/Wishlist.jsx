...
export default function WishlistPage() {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {items.length === 0 ? (
        <div className="text-center py-10 text-gray-400"> <HeartOff size={64} className="mx-auto mb-3" />
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {items.map((it) => (
            <div className="border p-3 rounded" key={it._id}>
              <img src={it.images?.[0] || it.variants?.[0]?.images?.[0]} alt={it.name}
                className="w-full h-36 object-cover rounded mb-2"
              />
              <div className="font-semibold">{it.name}</div>
              <div className="mt-2">
                Rs. {( it.price ?? it.basePrice ?? it.variants?.[0]?.price ?? 0 ).toFixed(2)}
              </div>
              <button onClick={() => remove(it._id)} className="mt-2 border px-3 py-1 rounded"
              >
                Remove
              </button>
